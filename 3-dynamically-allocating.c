...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        int num = 0;
        scanf("%d", &num);   //if user inputs negative number, loop will end and print out array
        if (num < 0) {
            break;
        }

        if (count >= num_elements) {    //check if # of elements = the # of count
            num_elements *= 2;          //double the # of the user input
            int *temp = (int *)realloc(array, num_elements * sizeof(int));  //new array called temp, reallocates og array to the new array size

            if (temp == NULL) {     //if new array = null, memory allocation failed, nothing inside old array
                printf("Memory reallocation failed.\n");
                free(array);        //free og array to prevent memory leaks
                return 1;
            }
            array = temp;   //move the pointer from og array to new array memory location
        }
        array[count] = num; //store entered numbers into array
        count++;
    }
